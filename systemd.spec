...
%{lua:
    for arch in string.gmatch(macros.ix86, '[^%s]+') do
        print(string.format('Requires: (systemd-boot if filesystem(%s))\n', arch))
    end
}
Requires:       (systemd-boot if filesystem(x86-64))
Requires:       (systemd-boot if filesystem(aarch64))
Requires:       (systemd-boot if filesystem(riscv64))
